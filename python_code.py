    ...
    def color_file(self):
        ...
        for line in self.file:
            ...
            for char in line:
                if (
                    not string_ignore
                    and not multiline_string_ignore
                    and not def_ignore
                    and not class_ignore
                ):
                    ...
                    if char == "'" or char == '"':
                        ...
                        if parens == 0:
                            ...
                            equals = False
                            if "->" not in word:
                                self.output += self.add_color(
                                    line, "lightblue", pos - len(word), pos
                                )
                                self.output += char
                                word = ""
                            else:
                                start = word.index("->") + 2
                                self.output += escape(word[:start]) + self.add_color(
                                    word, "lightgreen", start, len(word)
                                )
                                self.output += char
                                word = ""
                        else:
                ...
                # multiline
                    ...
                    #''' fgf ' add 1 to the apostrophe_end
                        ...
                        if apostrophe_end == 3:
                            ...
                            #'''derrere \n dere '''
                                ...
                                self.output += self.multiline_add_color_end(
                                    line, "orange", pos + 1
                                )
                                same_line = True
                    #' unclosed string so reset apostrophe_end to 0
                    ...
                    if char == "(" and not args:
                        ...
                        self.output += self.add_color(
                            line, "lightblue", pos - len(word), pos - 1
                        )
                        self.output += " ="
                        word = ""
                        equals = True
                        ...
                        if parens == 0:
                            if "->" not in word:
                                ...
                                start = word.index("->") + 2
                                self.output += escape(word[:start]) + self.add_color(
                                    word, "lightgreen", start, len(word) - 1
                                )
                                self.output += ")"
                                word = ""
                        args_done = True
                    ...
                    if char == ":":
                        self.output += self.add_color(
                            line, "lightgreen", pos - len(word), pos
                        )
                        self.output += escape(char)
                        word = ""
                        class_ignore = False
